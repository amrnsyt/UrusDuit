...
function muatDataBalanSemasa() {
    if (!masterDatabase.bulan[bulanAktif]) masterDatabase.bulan[bulanAktif] = janaStrukturBulanKosong();
...
function salinDataBulanLepas() {
    ...
    if(indexSemasa <= 0) return paparToast("Tiada Rekod", "Tiada bulan terdahulu untuk disalin.", "amaran");
    
    const bulanLepas = senaraiBulan[indexSemasa - 1];
    if(!masterDatabase.bulan[bulanLepas]) return paparToast("Tiada Data", `Bulan ${bulanLepas} belum mempunyai rekod yang sah.`, "amaran");
    
    bukaModal("Salin Data Bulan Lepas?", `Tindakan ini akan memadam data bulan ini and menggantikannya dengan rekod dari bulan ${bulanLepas}. Teruskan?`, "fa-clone", "bg-indigo-500/20", "text-indigo-500", "bg-indigo-600 hover:bg-indigo-700", "fa-check", "Ya, Salin", () => {
        ...
        if(dataSalinan.hutangCatPaid) { for(let key in dataSalinan.hutangCatPaid) dataSalinan.hutangCatPaid[key] = false; }
...
function terapkanTemaSemasa() {
    document.body.className = masterDatabase.isDarkMode ? "dark-theme min-h-screen flex flex-col justify-between pb-32" : "light-theme min-h-screen flex flex-col justify-between pb-32";
}
...
function bukaModal(tajuk, mesej, iconClass, wrapperClass, textIconClass, btnClass, iconBtnClass, btnText, action) {
    ...
    document.getElementById('modal-icon-wrapper').className = `w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${wrapperClass}`;
    document.getElementById('modal-icon').className = `fa-solid ${iconClass} ${textIconClass} text-xl`;
    ...
    btn.onclick = function() { if (modalConfirmAction) modalConfirmAction(); tutupModal(); };
